...
ENT.GLEE_IsPiggy = true

-- These can be overridden per-NPC if needed
ENT.NPCDisplayName = "Pig"
ENT.NPCCategory = "piggys"

local IsValid = IsValid

-- Helper to get sound path based on NPC class
function ENT:GetSoundPath(soundName)
	return Sound(self:GetClass() .. "/" .. soundName)
end

function ENT:GetJumpSound()
	return self:GetSoundPath("jump.mp3")
end

function ENT:GetJumpHighSound()
	return self:GetSoundPath("jumphigh.mp3")
end

function ENT:GetTauntSounds()
	return { self:GetSoundPath("attack.mp3") }
end

function ENT:GetChaseMusic()
	return Sound(self:GetClass() .. "/panic.mp3")
end

if SERVER then -- SERVER --

-- Shared convars for all Italian Brainrot NPCs
local npc_pig_acquire_distance =
	CreateConVar("npc_pig_acquire_distance", 9999, FCVAR_NONE,
	"The maximum distance at which Pig NPCs will chase a target.")

local npc_pig_spawn_protect =
	CreateConVar("npc_pig_spawn_protect", 0, FCVAR_NONE,
	"If set to 1, Italian Brainrot NPCs will not target players or hide within 200 units of \z
	a spawn point.")

local npc_pig_attack_distance =
	CreateConVar("npc_pig_attack_distance", 120, FCVAR_NONE,
	"The reach of the piggy's attack.")

local npc_pig_attack_interval =
	CreateConVar("npc_pig_attack_interval", 0.1, FCVAR_NONE,
	"The delay between Italian Brainrot NPC's attacks.")

local npc_pig_attack_force =
	CreateConVar("npc_pig_attack_force", 800, FCVAR_NONE,
	"The physical force of pig NPC's attack. Higher values throw things \z
	farther.")

local npc_pig_smash_props =
	CreateConVar("npc_pig_smash_props", 1, FCVAR_NONE,
	"If set to 1, pig NPCs will punch through any props placed in their way.")

local npc_pig_allow_jump =
	CreateConVar("npc_pig_allow_jump", 1, FCVAR_NONE,
	"If set to 1, pig NPCs will be able to jump.")

local npc_pig_hiding_scan_interval =
	CreateConVar("npc_pig_hiding_scan_interval", 3, FCVAR_NONE,
	"pig NPCs will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant NPCs are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_pig_hiding_repath_interval =
	CreateConVar("npc_pig_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to pig NPC's hiding spot will be redetermined every X seconds.")

local npc_pig_chase_repath_interval =
	CreateConVar("npc_pig_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of pig NPC's target will be redetermined every \z
	X seconds.")

local npc_pig_expensive_scan_interval =
	CreateConVar("npc_pig_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_pig_force_download =
	CreateConVar("npc_pig_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download Italian Brainrot NPC resources \z
...
if npc_pig_force_download:GetBool() then
	resource.AddWorkshop("3678265251")
end
...
local VECTOR_NPC_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_NPC_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_pig: found %d suitable (%d unsuitable) hiding \z
		places in %d areas over %.2fms!", goodSpots, badSpots, #areas,
		(SysTime() - rStart) * 1000))
end

local ai_ignoreplayers = GetConVar("ai_ignoreplayers")
local function isValidTarget(ent, selfClass)
	...
	-- Ignore dead NPCs, other Italian Brainrot NPCs, and dummy NPCs.
	local class = ent:GetClass()
	
	-- Check if it's an Italian Brainrot NPC
	local ispig = ent.GLEE_IsPiggy or false
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_pig_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		if not isValidTarget(ent, myClass) then continue end

		-- Spawn protection! Ignore players within 200 units of a spawn point
		-- if `npc_pig_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_pig_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_pig_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent, myClass) then
			...
			-- Give the player a good whack. NPC means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_pig_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_pig_attack_distance:GetInt()
	...
	then
		...
		self:EmitSound((jumpHeight > HIGH_JUMP_HEIGHT and
			self:GetJumpHighSound() or self:GetJumpSound()), 350, 100)
	end
end
...
function ENT:BehaveStart()
	...
	self.MovePath:SetMinLookAheadDistance(9999)
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_pig_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_pig_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_pig_attack_distance:GetInt()
			if self:AttackNearbyTargets(attackDistance) then
				if currentTime - self.LastTaunt > TAUNT_INTERVAL then
					...
					self:EmitSound(table.Random(self:GetTauntSounds()), 350, 100)
				end
		...
		local repathInterval = npc_pig_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_pig_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_pig_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_pig_hiding_repath_interval:GetFloat()
...
-- Get the material for this NPC based on class name
function ENT:GetNPCMaterial()
	local class = self:GetClass()
	-- Material path follows pattern: npc_X/X (e.g., npc_ballerina/ballerina)
	local npcName = string.sub(class, 5) -- Remove "npc_" prefix
	return Material(class .. "/" .. npcName)
end

-- Register kill icon for this NPC
function ENT:SetupKillIcon()
	local class = self:GetClass()
	local npcName = string.sub(class, 5) -- Remove "npc_" prefix
	killicon.Add(class, class .. "/killicon", color_white)
	language.Add(class, npcName)
end
...
local lastPanic = 0 -- The last time we were in music range of an NPC.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_pig_music_volume =
	CreateConVar("npc_pig_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by Italian Brainrot NPCs. (0-1, where 0 is muted)")

-- If another NPC comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, NPCs do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1500

-- Max volume is achieved when MUSIC_NPC_PANIC_COUNT NPCs are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 500

-- That's a lot of NPC.
local MUSIC_NPC_PANIC_COUNT = 2

local MUSIC_NPC_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_NPC_PANIC_COUNT

-- Store music instances per NPC class
local classMusicInstances = {}

local function updatePanicMusic()
	if not IsValid(LocalPlayer()) then return end
	
	local userVolume = math.Clamp(npc_pig_music_volume:GetFloat(), 0, 1)
	local myPos = LocalPlayer():GetPos()
	local playerAlive = LocalPlayer():Alive()
	
	-- Gather all nearby Italian Brainrot NPCs grouped by class
	local classBestScores = {}
	local anyNPCFound = false
	
	for _, ent in pairs(ents.GetAll()) do
		if IsValid(ent) and ent.GLEE_IsPiggy then
			anyNPCFound = true
			local class = ent:GetClass()
			local dist = myPos:Distance(ent:GetPos())
			local distanceScore = math.max(0, MUSIC_CUTOFF_DISTANCE - dist)
			
			if not classBestScores[class] then
				classBestScores[class] = {
					score = distanceScore,
					ent = ent
				}
			elseif distanceScore > classBestScores[class].score then
				classBestScores[class].score = distanceScore
				classBestScores[class].ent = ent
			end
		end
	end
	
	-- No NPCs found - stop everything
	if not anyNPCFound then
		DevPrint(4, "Halting music timer.")
		timer.Remove("pigPanicMusicUpdate")
		
		for class, musicData in pairs(classMusicInstances) do
			if musicData.sound then
	...
	-- Update music for each class
	for class, data in pairs(classBestScores) do
		local musicVolume = math.min(1, data.score / MUSIC_NPC_MAX_DISTANCE_SCORE)
		
		-- Create music instance for this class if needed
		if not classMusicInstances[class] then
			local chaseMusic = data.ent:GetChaseMusic()
			local sound = CreateSound(LocalPlayer(), chaseMusic)
			classMusicInstances[class] = {
				sound = sound,
				lastActive = CurTime()
			}
		end
		
		local musicData = classMusicInstances[class]
		musicData.lastActive = CurTime()
		...
		elseif musicVolume > 0 then
			...
			musicData.sound:ChangePitch(math.Clamp(game.GetTimeScale() * 100, 50, 255), 0)
			musicData.sound:ChangeVolume(musicVolume, 0)
		else
			musicData.sound:ChangeVolume(0, 0)
		end
	end
	
	-- Stop music for classes no longer present
	for class, musicData in pairs(classMusicInstances) do
		if not classBestScores[class] then
			if CurTime() - musicData.lastActive >= MUSIC_RESTART_DELAY then
				musicData.sound:Stop()
				classMusicInstances[class] = nil
			else
				musicData.sound:ChangeVolume(0, 0)
			end
		end
...
local function startTimer()
	if not timer.Exists("pigPanicMusicUpdate") then
		timer.Create("pigPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:Initialize()
	...
	-- Setup kill icon for this specific NPC
	self:SetupKillIcon()
...
function ENT:DrawTranslucent()
	render.SetMaterial(self:GetNPCMaterial())

	-- Get the normal vector from NPC to the player's eyes, and then compute
	...
	-- NPC should only look 1/3 of the way up to the player so that they
